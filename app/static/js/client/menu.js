...
class MenuManager {
    static translations = {
        ...
        'tk': {
            ...
            'cart-title': 'Sebet',
            'clear-cart': 'Arassala',
            'empty-cart': 'Sebet boş, ýöne deňiz doly.\nMenýa serediň - ol ýerde tagam tolkunlary bar.',
            'loading': 'Menýu ýüklenýär...',
            'error-title': 'Menýu ýüklemek ýalňyşlygy',
            'error-button': 'Täzele',
            'empty-title': 'Hiç zat tapylmady',
            'empty-subtitle': 'Gözleg sözüni üýtgetmegi synanyň',
            'call-waiter': 'Ofisianty çagyr',
            'table-label': 'Stol #',
            ...
            'table-selected': 'Stol saýlandy #',
            'bonus-card-applied': 'Bonus karta ulanyldy!',
        ...
        'en': {
            ...
            'cart-title': 'Cart',
            'clear-cart': 'Clear',
            'empty-cart': 'Cart is empty, but the sea is full.\nCheck out the menu — there are waves of flavor.',
    ...
    static renderCategories() {
        ...
        // Добавляем "Все меню"
        const t = this.translations[this.currentLanguage] || this.translations['ru'];
        const allMenuHTML = `
            <div class="category-item ${!this.currentCategory ? 'active' : ''}" 
                 data-category-id="all"
                 onclick="MenuManager.selectCategory(null)">
                <div class="category-name">${t['all-menu']}</div>
    ...
    static updateInterfaceTexts(language) {
        const texts = this.translations[language] || this.translations['ru'];
        ...
        const allMenuText = document.getElementById('allMenuText');
        if (allMenuText) {
            allMenuText.textContent = texts['all-menu'];
        }
